...
def sign_up(event):
    ...
    if len(password) < 10:
        output_div.className = "error"
        output_div.innerHTML = "<h3>❌ Password must be at least 10 characters!!!!!</h3>"
        return
    
    if len(fullname) < 7:
        output_div.className = "error"
        output_div.innerHTML = "<h3>❌ Full name must be at least 7 characters long!</h3>"
        return
    ...
    if not (has_letter and has_number):
        output_div.className = "error"
        output_div.innerHTML = "<h3>❌ Password must contain at least one letter and one number!</h3>"
        return
...
#For sake of coders working on this project:
